...
After [Chapter 1](chapter_01_random_variables/random_variables.html), you should be able to

1) Identify random variables for basic things that we want to model.  
2) Start to build richer models with basic random variables. 
...
After [the chapter on Monte Carlo](chapter_02_monte_carlo/monte_carlo.html), you should know how to compute 
(1) probabilities $P(X \in A)$ as frequencies
(2) expectations $\mathbb{E}(X)$ as averages and
(3) "distributions" as histograms 
by simulating the random variable $X$.  These techniques are referred to as Monte Carlo simulation.
...
After [the logic of statistical testing via Monte Carlo Simulation](LogicOfStatisticalTesting.html), you should know how to test a hypothesis with Monte Carlo.  This involves three steps.   
1) Convert null hypothesis into a statistical model from which we can simulate (recall chapter 1).  
2) Develop a test statistic $T$ and "surprising set" for $T$ based upon observed data and our understanding of the setting.   
3) Compute $P(T \in SurprisingSet)$ with Monte Carlo to get a p-value (recall chapter 2).  
